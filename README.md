...
python -m pip install \
  https://github.com/bulltickr/impactprism/releases/download/v0.4.6/impactprism-0.4.6-py3-none-any.whl
impactprism scan .
...
```bash
gh attestation verify impactprism-0.4.6-py3-none-any.whl \
...
| `scan` | `<repo>` — repository to scan | `--exclude PAT` (repeatable), `--root PATH` (repeatable for npm/pnpm package or Go module roots), `--sbom PATH`, `--report PATH`, `--evidence PATH`, `--json` |
| `diff` | `<current_report> <baseline_report>` | `--json` |
| `analyze` | `<repo_dir>` — repository to analyze | `--exclude PAT` (repeatable), `--root PATH` (repeatable for npm/pnpm package or Go module roots), `--sbom PATH`, `--report PATH`, `--json` |
...
For npm/pnpm monorepos, `--root apps/web --root packages/shared` selects literal
package directories containing `package.json`. For Go workspaces or
multi-module repositories, `--root apps/app --root libs/shared` selects literal
module directories containing `go.mod`. The same values can be supplied through
`[scan].roots` or the Action's newline-separated `roots` input. Root selection
is explicit and reportable; it does not accept globs, does not apply to Python,
and keeps the repository as the dependency-resolution context. The canonical
report records selected roots under `scope.roots`.
