  ...
  impactprism:
    ...
    steps:
      ...
      - name: ImpactPrism dependency-drift scan
        id: impactprism
        uses: bulltickr/impactprism@v0.4.8
        with:
...
| `roots`              | Newline-separated repository-relative npm/pnpm package or Go module roots; npm roots contain `package.json`, Go roots contain `go.mod`, and globs are not accepted. | false | empty |
...
- `ecosystem: auto` detects `package.json` (npm), `go.mod`/`go.work` (Go), or a supported
...
- `roots` is an opt-in npm/pnpm package or Go module selection. The repository
  remains the workspace and dependency-resolution context, while only the
  selected package/module manifests and source trees are classified. Omit it
  for the historical whole-repository scan.
...
The generated `findings.json` policy object records `fail_on`,
`severity_threshold`, `gate_source`, `outcome`, `exit_code`, and the number of
findings considered and triggered. With `baseline-path`, the gate source is
`baseline-new-findings`: the full current finding set remains in the report,
but only newly introduced findings determine the policy result.
...
`bulltickr/impactprism@v0.4.8`. Action releases use git tags matching
...
`src/impactprism/version.py`; the current synchronized release is `v0.4.8`,
