...
def _run_analyze(args):
    ...
    ecosystem = args.ecosystem if args.ecosystem != "auto" else _detect_ecosystem(repo_path)
    ...
    try:
        ...
        if args.sbom is not None and result.sbom is not None:
            _write_json(args.sbom, result.sbom)
        if args.json:
            json.dump(report, sys.stdout, indent=2)
            print()
        else:
            scanner_error_message = result.scanner_error_message
...
def _run_scan(args):
    ...
    try:
        ...
        if sbom_arg is not None and result.sbom is not None:
            _write_json(sbom_arg, result.sbom)
...
def main(argv=None) -> int:
    ...
    scan.add_argument("--severity-threshold", choices=("info", "low", "medium", "high", "critical"), default="low")
