...
Download the files from [v0.4.8](https://github.com/bulltickr/impactprism/releases/tag/v0.4.8),
download `SHA256SUMS`, and verify the artifacts before installing:

```bash
sha256sum -c SHA256SUMS
python -m pip install ./impactprism-0.4.8-py3-none-any.whl
```

On Windows PowerShell, the equivalent checksum command is:

```powershell
Get-FileHash .\impactprism-0.4.8-py3-none-any.whl -Algorithm SHA256
```
